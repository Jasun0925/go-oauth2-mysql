...
func (s *TokenStore) Create(ctx context.Context, info oauth2.TokenInfo) error {
	...
	if code := info.GetCode(); code != "" {
		...
		// TODO 将UserId与token绑定
		s2 := struct {
			AccessToken  string
			RefreshToken string
		}{info.GetAccess(), info.GetRefresh()}
		cbinfo, err := jsonMarshal(s2)
		if err != nil {
			return err
		}
		pipe.Set(ctx, s.wrapperKey(info.GetUserID()), cbinfo, rexp)
	}
...
// GetTokenByClientId 返回token
func (s *TokenStore) GetTokenByUserId(ctx context.Context, userId string) string {
	result := s.cli.Get(ctx, s.wrapperKey(userId))
	return result.Val()
}
