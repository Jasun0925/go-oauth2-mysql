...
type Manager struct {
	...
	// TODO 是否使用Bcrypt加密Secret
	isClientSecretBcrypt bool
}
...
// MapClientBcryptStorage TODO 使用bcrypt加密了Secret
func (m *Manager) MapClientBcryptSecretStorage(stor oauth2.ClientStore, b bool) {
	m.clientStore = stor
	m.isClientSecretBcrypt = b
}
...
// GenerateAccessToken generate the access token 生成访问令牌
func (m *Manager) GenerateAccessToken(ctx context.Context, gt oauth2.GrantType, tgr *oauth2.TokenGenerateRequest) (oauth2.TokenInfo, error) {
	...
	if cliPass, ok := cli.(oauth2.ClientPasswordVerifier); ok {
		...
		// TODO 在这里进行对比secret的
	} else if len(cli.GetSecret()) > 0 {
		if !m.isClientSecretBcrypt {
			if tgr.ClientSecret != cli.GetSecret() {
				return nil, errors.ErrInvalidClient
			}
		} else {
			// TODO 比较哈希值和密码
			err = bcrypt.CompareHashAndPassword([]byte(cli.GetSecret()), []byte(tgr.ClientSecret))
			if err != nil {
				return nil, errors.ErrInvalidClient
			}
		}
	...
	// TODO 上面的所有client信息对比完成后，通过userId，拿到token
	s2 := struct {
		AccessToken  string
		RefreshToken string
	}{}
	tokenByClientId := m.tokenStore.GetTokenByUserId(ctx, tgr.UserID)
	err = json.Unmarshal([]byte(tokenByClientId), &s2)
	if s2.AccessToken != "" {
		ti.SetAccess(s2.AccessToken)
		ti.SetRefresh(s2.RefreshToken)
		return ti, nil
	}
